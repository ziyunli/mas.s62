...
// methods on existing structs, especially in the forge.go file.
...
// ToHex gives a hex string for a PublicKey. no newline at the end
func (secKey SecretKey) ToHex() string {
	// format is zerohash 0...255, onehash 0...255
	var s string
	for _, zero := range secKey.ZeroPre {
		s += zero.ToHex()
	}
	for _, one := range secKey.OnePre {
		s += one.ToHex()
	}
	return s
}
...
func (m Message) IsBitSet(i int) bool {
	return m[i/8]>>(7-(i%8))&0x01 == 1
}
